...
class MembershipDTO : BaseDTO() {
    ...
    private val memberType: @NotNull(groups = [Administration::class]) MemberType? = null

    @NotNull(groups = [Creation::class])
    private val city: @NotNull(groups = [Creation::class]) String? = null

    @NotNull(groups = [Creation::class])
    private val country: @NotNull(groups = [Creation::class]) String? = null

    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
    @PastOrPresent(groups = [Administration::class])
    @Today(groups = [Creation::class])
    private val startDate: @PastOrPresent(groups = [Administration::class]) LocalDate? = null
    ...
    @JsonProperty
    @NotNull
    private val incasso: @NotNull Boolean = false
}
