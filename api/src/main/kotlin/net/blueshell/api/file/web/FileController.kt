...
import net.blueshell.api.file.repository.FileRepository
import net.blueshell.api.file.service.FileService
...
class FileController(service: FileService, repository: FileRepository, private val fileMapper: FileMapper) :
    ...
    fun uploadEventBanner(
        @RequestPart("file") @NotNull(message = "File is required") @net.blueshell.api.file.validation.FileSize(max = 2 * 1024 * 1024) @net.blueshell.api.file.validation.AllowedContentTypes(
            "image/png",
            "image/jpeg",
            "image/jpg",
            "image/webp",
