...
class UpdateEventInCalendarJob {
    ...
    fun update(eventId: Long?): CompletableFuture<Void?> {
        ...
        try {
            ...
            if (e == null) {
                UpdateEventInCalendarJob.log.warn("Update skipped: eventId {} not found", eventId)
            ...
            if (!e.isApproved()) {
                ...
                if (e.getGoogleId() != null) {
                    ...
                    UpdateEventInCalendarJob.log.info("EventId {} not approved anymore -> removed from Google", eventId)
                } else {
                    UpdateEventInCalendarJob.log.info("EventId {} not approved and not on Google -> noop", eventId)
                }
                return CompletableFuture.completedFuture<Void?>(null)
            }
            ...
            if (e.getGoogleId() == null) {
                ...
                eventService.update(e)
                UpdateEventInCalendarJob.log.info(
    ...
    fun recover(ex: Exception, eventId: Long?): CompletableFuture<Void?> {
        ...
        UpdateEventInCalendarJob.log.error("Giving up update for eventId {}: {}", eventId, ex.message, ex)
        return CompletableFuture.completedFuture<Void?>(null)
    }
