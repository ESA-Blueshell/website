...
ALTER TABLE `events`
    DROP FOREIGN KEY `fk_events_survey_id`,
    ADD CONSTRAINT `fk_events_survey_id`
        FOREIGN KEY (`survey_id`) REFERENCES `surveys`(`id`)
            ON UPDATE RESTRICT ON DELETE RESTRICT;

/* Existing name `fk_events_banner_file` is already lowercase and follows fk_<table>_<column>.
   If you want to force `_id` style but the column is not present in the current model, leave as-is. */
