...
class ValidAnswerValidatorTest {
    ...
    fun `rejects when question id is missing or unknown`() {
        assertThat(validator.isValid(AnswerDTO(questionId = null), mock())).isFalse()

        whenever(questions.findById(999)).thenThrow(RuntimeException("not found"))
        assertThat(validator.isValid(AnswerDTO(questionId = 999), mock())).isFalse()
    }
    ...
    fun `validates open text answers`() {
        ...
        assertThat(validator.isValid(AnswerDTO(questionId = 1, textResponse = "hello"), mock())).isTrue()
        assertThat(validator.isValid(AnswerDTO(questionId = 1, textResponse = "  "), mock())).isFalse()
    }
    ...
    fun `validates checkbox selection size against labels`() {
        ...
        val valid = AnswerDTO(questionId = 2, optionSelections = mutableListOf(true, false, true))
        val wrongSize = AnswerDTO(questionId = 2, optionSelections = mutableListOf(true, false))
        val missingSelections = AnswerDTO(questionId = 2, optionSelections = null)
    ...
    fun `validates radio selection count and size`() {
        ...
        val exactlyOne = AnswerDTO(questionId = 3, optionSelections = mutableListOf(true, false))
        val none = AnswerDTO(questionId = 3, optionSelections = mutableListOf(false, false))
        val multiple = AnswerDTO(questionId = 3, optionSelections = mutableListOf(true, true))
        val wrongSize = AnswerDTO(questionId = 3, optionSelections = mutableListOf(true, false, false))
    ...
    fun `description question always passes`() {
        ...
        val dto = AnswerDTO(questionId = 4, optionSelections = null, textResponse = null)
