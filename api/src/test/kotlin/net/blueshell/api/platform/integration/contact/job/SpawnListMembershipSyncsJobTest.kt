...
import net.blueshell.api.platform.integration.contact.application.job.SpawnListMembershipSyncsJob
...
/**
 * Unit tests for [SpawnListMembershipSyncsJob].
 *
 * No Spring context — instantiate directly with mocks.
 */
class SpawnListMembershipSyncsJobTest {
    ...
    private fun adapterFor(system: ContactSystem): ListAdapter = mock<ListAdapter>().also {
    ...
    fun `enqueues one SyncListMembershipForSystem job per membership per adapter`() {
        ...
        val job = SpawnListMembershipSyncsJob(objectMapper, contactListMembershipRepository, listOf(adapter1, adapter2), jobs)
        ...
        job.handle(objectMapper.writeValueAsString(ContactJobs.SpawnListMembershipSyncsPayload()))
    ...
    fun `does nothing when no memberships exist`() {
        ...
        val job = SpawnListMembershipSyncsJob(objectMapper, contactListMembershipRepository, listOf(adapter), jobs)
