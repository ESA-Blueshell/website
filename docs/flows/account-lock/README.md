...
|------|--------|---------------|---------|----------|
| `/recovery/lock` | POST | permit all | lock link | 204; the same for a used, expired or unknown link |
...
| Concern | Location |
|---------|----------|
| Security notifications | `services/api/.../security/` (new), on the pattern of `auth/domain/RoleChangeEmailBuilder.kt` |
| Lock, unlock and email change | `services/api/.../auth/domain/` (new), token purposes in `shared/enums/TokenPurpose.kt` |
| Contact links | `services/api/.../discord/domain/DiscordDoors.kt` |
| Lock page | `services/frontend/src/pages/account/` (new) |
| Unlock on the row | `services/frontend/src/components/common/rows/UserManagerRow.vue` |

## 11. Testing

| Invariant | Suite |
|-----------|-------|
| Each change sends its notification, with a lock link | api integration, reading the in-memory outbox |
| A lock link only locks, once, within seventy-two hours | api integration and `account-lock.feature` (new) |
| Lock links do not retire each other | api integration |
| A lock ends sign-ins, forgets browsers and drops a pending change | api integration |
| Unlock is admin-only and needs a reason | api integration |
| An email change needs the new address and tells the old | `account-lock.feature` (new), links read from `/test-support/emails` |
| Email change and lock, driven through a browser | browser system test under `tests/system/.../frontend/account/` (new) |
| Lock page and the change-email form | frontend unit tests |
