...
seconds so that calls already in flight, and other tabs, are not refused. Opening the
site always rotates.
...
two-factor it always asks for a fresh code before issuing an authorization code, and a
trusted browser does not skip it. The ID token names the methods used in `amr`:
`pwd`, and `otp` once a code has been given.
...
| Challenge, code check and trusted browsers | `twofactor/` (new) |
| Sign-in record, rotation, reuse and binding | `security/` (new sign-in store beside `AuthTokenCookieService.kt`) |
...
| OIDC and forward-auth | `oidc/domain/AuthorizationServerConfig.kt`, `oidc/web/ForwardAuthController.kt` |

**Frontend** — under `services/frontend/src/`

| Concern | Location |
|---------|----------|
| The page and its code step | `pages/login/Login.vue` |
| Session state | `plugins/store.ts` — holds who signed in and their roles in force, never a token |
| Which routes need a sign-in, and the forced set-up | `plugins/router.ts` |
| Cookie and CSRF handling | `services/api/blueshell.runtime.ts` |

## Testing

| Suite | Location | Covers |
|-------|----------|--------|
| Acceptance features | [`sign-in.feature`](../../../tests/system/src/test/resources/features/sign-in.feature) | Who gets in, who is challenged and what a refusal gives away |
| Browser system tests | `tests/system/.../frontend/login/LoginPageSystemTest.kt` | The page as a user drives it, code step included |
...
| API integration | `services/api/src/integrationTest/.../auth/web/` | Challenge is not a sign-in; replay; five tries; per-account limit across replicas; lock after password |
| API integration | `services/api/src/integrationTest/.../security/` | Rotation and grace; reuse ends the sign-in; another browser ends it; thirty and fourteen days |
| API integration | `services/api/src/integrationTest/.../oidc/web/ForwardAuthControllerIT.kt` | A dormant role opens nothing; OIDC asks for a fresh code; a refresh token dies with its sign-in |
| Frontend unit | `services/frontend/tests/unit/pages/login/Login.test.ts` | Form behaviour, the code step and error rendering |
