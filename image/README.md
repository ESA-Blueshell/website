...
├── cloud-init/
│   ├── cloud-config-standalone.template.yaml  Cloud-init template
...
| `CONTABO_PRODUCT_ID` | VPS product ID, e.g. `V22` (run `cntb get products` to list) |
...
| `REMOTE_HOST` | Server IP or hostname (set after step 5) |
...
The admin key must be registered with Contabo before creating the VPS.
...
> This step only needs to be done once. Reuse the same key ID for future servers.
...
This substitutes all passwords, SSH public keys, GHCR credentials, and
auto-generated secrets into the template. The rendered files contain secrets —
do not commit them.
...
After the command returns, find the assigned IP in the Contabo portal (or via
`cntb get instances`) and set it in `.env`:
...
- The rendered `cloud-config-standalone.yaml` contains secrets — do not commit it to the repository.
