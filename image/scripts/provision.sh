...
OUT="/src/backups/db-backup-${TIMESTAMP}.sql.gz"
...
if [[ ! -s "${OUT}" ]]; then
  echo "Error: backup output is empty: ${OUT}" >&2
  exit 1
fi

chmod 640 "${OUT}"
chown root:backup "${OUT}"

# 2) Environment files snapshot
rsync -a --delete /src/website/env/ /src/backups/env/
...
find "/src/backups" -maxdepth 1 -type f -name 'db-backup-*.sql.gz' -mtime +7 -delete
