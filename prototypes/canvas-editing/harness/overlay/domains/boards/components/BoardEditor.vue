...
const accent = computed(() => colour.value.trim() || "var(--color-brand)")
const complete = computed(() => Number.isInteger(numbered.value) && numbered.value > 0 && startDate.value !== "")
...
<template>
  ...
  >
    ...
    >
      ...
      <p
        v-if="failure"
        class="board-editor__failure"
        data-testid="board-edit-failure"
        role="alert"
      >
        {{ failure }}
      </p>
