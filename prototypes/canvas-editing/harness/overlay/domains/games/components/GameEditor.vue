...
/** The esports channels, which are separate from the channels casual players meet in. */
const esportsChannels = ref<GameChannel[]>([])
...
const backLabel = computed(() => ({casual: "Casual", competition: "Competition", games: "Games"})[props.area])
    ...
    >
      ...
      >
        <vv-field
          ...
          :component-props="{kind: 'textarea', hint: 'Empty uses the casual intro'}"
          label="Intro"
      ...
      <div class="game-previews">
        ...
        >
          ...
          <preview-frame>
            ...
            >
              <p
                v-if="esportsChannels.length > 0"
                class="game-previews__channels"
              >
                <span>Esports on Discord</span> {{ esportsLine }}
              </p>
            </esports-game-head>
          </preview-frame>
          <preview-frame>
...
.game-previews__channels {
  margin-top: 1rem;
  font-size: 0.9rem;
  color: var(--color-chalk);
}

.game-previews__channels span {
  margin-right: 0.5rem;
  font-size: 11px;
