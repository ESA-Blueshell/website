...
class EsportsControllerIT : UserTestSupport() {
    ...
    private fun season(
        ...
        from: LocalDate,
        to: LocalDate,
    ): Season = seasons.save(Season(name = name, startDate = from, endDate = to))
    ...
    private fun team(
        game: String,
        name: String,
    ): Squad = Squad(teams.save(Team(name = name)), game)
    ...
    inner class PublicRead {
        ...
        fun `shows the newest season's roster when none is asked for`() {
            ...
            mvc
                .perform(get("/esports/games/{game}", "TRACKMANIA"))
        ...
        fun `shows an older season when asked for it`() {
            ...
            mvc
                .perform(get("/esports/games/{game}", "GEOGUESSR").param("seasonId", older.id.toString()))
                .andExpect(status().isOk)
        ...
        fun `never publishes a real name, however the entry was recovered`() {
            ...
            mvc
                .perform(get("/esports/games/{game}", "SMASH"))
                .andExpect(status().isOk)
