...
class InboundReconcile(
    ...
    fun preview(
        subjectId: Long,
        cohortId: Long,
    ): InboundReconcilePreview = preview(loadTarget(subjectId, cohortId))
    ...
    fun apply(
        subjectId: Long,
        cohortId: Long,
        request: InboundReconcileApplyRequest,
    ...
    fun applyJob(payload: CohortJobs.ApplyInboundReconcilePayload): List<ApplyInboundReconcileItemResult> {
        val definition =
            definitions.byKey(payload.definitionKey)
    ...
    private fun loadTarget(
        ...
        val definition =
            subject.definitionKey?.let { definitions.byKey(it) }
    ...
    ): Pair<List<InboundReconcileRow>, List<InboundReconcileRow>> {
        val duplicateIds =
            remote
                .groupingBy { it.externalUserId }
                .eachCount()
                .filterValues { it > 1 }
                .keys
    ...
    private fun internalExternalIds(target: InboundTarget): Set<String> {
        ...
        return externalIds
            .findBatch(ExternalIdMappingService.USER_AGGREGATE, internalIds, target.system.name)
    ...
    private fun InboundTarget.matches(
        payload: CohortJobs.ApplyInboundReconcilePayload,
        definition: CohortDefinition,
    ...
    private fun PlatformTransactionManager.tx(propagation: Int) = TransactionTemplate(this).apply { propagationBehavior = propagation }
}
...
data class InboundReconcileApplyRequest(
    val previewToken: String,
    val selectedExternalUserIds: List<String>,
)

data class InboundReconcileApplyResponse(
    val jobId: Long?,
    val acceptedCount: Int,
    val skippedCount: Int,
)

data class ApplyInboundReconcileItemResult(
    val externalUserId: String,
    val userId: Long,
    val status: MembershipWriteStatus,
)
