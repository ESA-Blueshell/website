 ...
 *
 * The team and the season come with [teamSeason] rather than being named again, so a line-up
 * for a team nobody fielded is unrepresentable rather than merely discouraged. `userId` is
 * nullable, since most recovered history is a handle and nothing else, and a plain
 * Long, so a roster never walks the user entity graph. [handle] is what the entry was published
 * under: a linked entry prefers the user's current handle, leaving this the historical record.
 */
@Entity
@Table(
