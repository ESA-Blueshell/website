...
class FileService
    ...
    constructor(
        ...
        fun store(
            ...
            try {
                return scratch.hold(content).use { staged ->
                    ...
                    val prepared =
                        if (type.admitsVector && SvgUploads.isDeclared(declaredMediaType)) {
        ...
        private fun storeBytes(
            ...
            val entity =
                repository.findByPath(key).orElse(null) ?: File(
                    name = originalName,
                    path = key,
                    uploader = uploader,
                    mediaType = mediaType,
        ...
        fun findPublicImage(
            path: String,
            type: FileType,
        ): File? = repository.findByPath(path).orElse(null)?.takeIf { it.type == type && it.type.publiclyReadable }
        ...
        fun findByBannerEventId(eventId: Long): File {
            val fileId =
                eventBannerFiles.fileIdForEvent(eventId)
