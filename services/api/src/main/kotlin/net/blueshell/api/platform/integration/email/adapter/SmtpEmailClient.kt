 ...
 *
 * Generates a UUID-based Message-ID locally so the same value can both be
 * persisted on the outbox row and emitted as the `Message-ID:` MIME header —
 * giving the bounce poller a stable identifier to correlate DSNs back to
 * outbox entries.
 *
...
class SmtpEmailClient(
    ...
    override fun send(
        ...
        // The Message-ID header has to be set on the underlying MimeMessage
        // *before* MimeMessageHelper writes any other headers, otherwise
        // Jakarta Mail will synthesise its own at send time.
        ...
        // Re-assert the header — MimeMessageHelper's setters can rewrite headers.
        mime.setHeader("Message-ID", messageId)
