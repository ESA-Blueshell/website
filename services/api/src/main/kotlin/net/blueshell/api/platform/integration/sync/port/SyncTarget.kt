...
/** A system this app pushes aggregate state to. Persisted as a string in [external_id_mapping.system]. */
enum class TargetSystem { LISTMONK, BREVO, GOOGLE_CALENDAR }

/** The kind of aggregate a target syncs. Persisted as a string in [external_id_mapping.aggregate_type]. */
enum class AggregateType { USER, EVENT, CONTACT_LIST }

/**
 * Pushes one aggregate's current state to one external system.
 *
 * Contract: `data == null` means "this aggregate should not exist in the external system".
 * Return value is the external id after the push, or `null` if the aggregate was removed.
 */
