...
class MembershipService
    ...
    constructor(
        ...
        fun existsByUserId(userId: Long): Boolean = repository.existsByUser_Id(userId)

        fun existsActiveMembershipByUserId(userId: Long): Boolean = repository.existsByUser_IdAndEndDateIsNull(userId)

        fun findByUserId(userId: Long): MutableList<Membership> = repository.findByUser_Id(userId)
        ...
        fun findUserIdsOverlapping(
            from: LocalDate,
            to: LocalDate,
        ): Set<Long> = repository.findUserIdsOverlapping(from, to).toSet()
        ...
        fun findOverlappingWithMembers(
            from: LocalDate,
            to: LocalDate,
        ): List<Membership> = repository.findOverlappingWithMembers(from, to)

        @Transactional(readOnly = true)
        fun heldMembershipBetween(
            userId: Long,
            from: LocalDate,
            to: LocalDate,
        ): Boolean = repository.existsOverlapping(userId, from, to)
