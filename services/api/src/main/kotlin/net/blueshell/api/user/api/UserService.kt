...
class UserService
    ...
    constructor(
        ...
        fun loadUserPrincipalByUsername(username: String): UserPrincipal = UserPrincipalMapper.fromUser(findByUsername(username))
        ...
        fun existsByUsernameAndIdNot(
            username: String,
            id: Long,
        ): Boolean = repository.existsByUsernameAndIdNot(username, id)
        ...
        fun existsByEmailAndIdNot(
            email: String,
            id: Long,
        ): Boolean = repository.existsByEmailAndIdNot(email, id)

        fun existsByPhoneNumber(phoneNumber: String): Boolean = repository.existsByPhoneNumber(phoneNumber)

        fun existsByPhoneNumberAndIdNot(
            phoneNumber: String,
            id: Long,
        ): Boolean = repository.existsByPhoneNumberAndIdNot(phoneNumber, id)

        fun existsByDiscord(discord: String): Boolean = repository.existsByDiscord(discord)

        fun existsByDiscordAndIdNot(
            discord: String,
            id: Long,
        ): Boolean = repository.existsByDiscordAndIdNot(discord, id)
        ...
        fun findAllByIds(ids: Collection<Long>): List<User> = if (ids.isEmpty()) emptyList() else repository.findAllById(ids).toList()
         ...
         */
        fun isSoftDeleted(userId: Long): Boolean = repository.findSoftDeletedUserId(userId) != null
        ...
        fun findActiveIdsAfter(
            afterId: Long,
            limit: Int,
        ): List<Long> = repository.findActiveIdsAfter(afterId, PageRequest.of(0, limit))
        ...
        private fun withDefaultOrder(pageable: Pageable): Pageable =
            when {
