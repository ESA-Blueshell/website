...
class MembershipController(
    ...
    fun findMemberships(
        @ParameterObject query: MembershipQuery = MembershipQuery(),
    ): MutableList<MembershipResponse> = useCases.findByQuery(query).map { it.asResponse() }.toMutableList()
    ...
    ): MembershipResponse = useCases.end(id).asResponse()

    @PreAuthorize("hasPermission(#id, 'Membership', 'write')")
    @PostMapping(value = ["/memberships/{id}/reopen"])
    fun reopenMembership(
        @PathVariable @Positive id: Long,
    ): MembershipResponse = useCases.reopen(id).asResponse()

    @PreAuthorize("hasPermission(#id, 'Membership', 'read')")
    @GetMapping(value = ["/memberships/{id}"])
    fun findMembershipById(
        @PathVariable @Positive id: Long,
    ): MembershipResponse = useCases.findById(id).asResponse()
    ...
    fun restoreMembership(
        @PathVariable @Positive id: Long,
    ): MembershipResponse = useCases.restore(id).asResponse()

    @PreAuthorize("hasPermission('__NO_TARGET__', 'Membership', 'read-deleted')")
    @GetMapping(value = ["/users/{userId}/memberships/deleted"])
    fun findDeletedMemberships(
        @PathVariable @Positive userId: Long,
    ): MutableList<MembershipResponse> = useCases.findDeletedByUserId(userId).map { it.asResponse() }.toMutableList()
}
