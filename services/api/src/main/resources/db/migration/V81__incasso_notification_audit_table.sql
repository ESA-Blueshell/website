-- Create incasso_notifications audit table for bulk incasso notification.
-- Mirrors contribution_reminders: composite (user_id, contribution_period_id)
-- identity, soft-delete sentinel, optimistic-lock version, and audit FKs.
CREATE TABLE incasso_notifications
(
    user_id                BIGINT                                 NOT NULL,
    contribution_period_id BIGINT                                 NOT NULL,
    amount                 DOUBLE                                 NULL,
    expected_incasso_date  DATE                                   NULL,
