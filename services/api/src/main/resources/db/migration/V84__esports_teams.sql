...
CREATE TABLE team_roster_entry (
    ...
    updated_by_id BIGINT       NULL,
    deleted_at    DATETIME(6)  NOT NULL DEFAULT '9999-12-31 23:59:59.000000',
    -- One appearance per person per team per season, whether they are named by a user link
    -- or only by the handle they were published under.
    UNIQUE INDEX uk_roster_entry       (team_id, season_id, handle, deleted_at),
    INDEX idx_roster_entry_season      (season_id, deleted_at),
    INDEX idx_roster_entry_user        (user_id, deleted_at),
    INDEX idx_roster_entry_deleted_at  (deleted_at),
    CONSTRAINT fk_roster_entry_team    FOREIGN KEY (team_id)       REFERENCES team   (id),
    CONSTRAINT fk_roster_entry_season  FOREIGN KEY (season_id)     REFERENCES season (id),
