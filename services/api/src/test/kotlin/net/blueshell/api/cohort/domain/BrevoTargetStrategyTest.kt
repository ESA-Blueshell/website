...
class BrevoTargetStrategyTest {
    ...
    fun `says where each list sits, outside in`() {
        whenever(contactsApi.getFolders(eq(50L), eq(0L), eq(GetContactsSortParameter.ASC)))
            .thenReturn(GetFolders200Response(count = 1L, folders = listOf(folder(1L, "Committees"))))
        whenever(contactsApi.getLists(eq(50L), eq(0L), eq(GetContactsSortParameter.ASC)))
            .thenReturn(
                GetLists200Response(count = 2L, lists = listOf(list(10L, "Web Cmte", 1L), list(11L, "Loose ends", 404L))),
            )
    ...
    fun `filters by query after fetching the bounded catalog`() {
        whenever(contactsApi.getFolders(eq(50L), eq(0L), eq(GetContactsSortParameter.ASC)))
            .thenReturn(GetFolders200Response(count = 1L, folders = listOf(folder(1L, "Members"))))
        whenever(contactsApi.getLists(eq(50L), eq(0L), eq(GetContactsSortParameter.ASC)))
            .thenReturn(GetLists200Response(count = 2L, lists = listOf(list(10L, "Guests", 1L), list(11L, "Paid", 1L))))
    ...
    // The generated models are immutable data classes, so the counts Brevo
    // always returns have to be supplied even where the assertions ignore them.
