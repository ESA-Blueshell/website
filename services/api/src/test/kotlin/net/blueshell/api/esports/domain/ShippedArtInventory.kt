...
class ShippedArtInventory(
    ...
    private fun exists(art: String): Boolean = javaClass.classLoader.getResource("${seed.directory}/art/$art.webp") != null
    ...
    private fun text(
        ...
        at: Int,
        length: Int,
    ) = String(bytes, at, length, US_ASCII)

    private fun le(
        bytes: ByteArray,
        at: Int,
        length: Int,
    ): Int = (0 until length).sumOf { (bytes[at + it].toInt() and 0xFF) shl (8 * it) }
