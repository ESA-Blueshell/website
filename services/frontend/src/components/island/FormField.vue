...
/* What every island field shares: its name, its message, and the room kept for that message. */
...
const {
  ...
  label: string
  /** Inside, the label rests on the line and rises once the control holds something. */
  variant?: "stacked" | "inside"
  /** The control holds something, so an inside label stays risen. */
  filled?: boolean
  /** Said under the control, where the error takes its place. */
  hint?: string
  error?: string
  required?: boolean
  testid?: string
...
.island-field--inside .island-field__label {
  position: absolute;
  /* Centred rather than set at a fixed height: the box is as tall as whatever
