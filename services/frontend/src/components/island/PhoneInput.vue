...
/* One box for the country and the number; the form keeps E.164, which is what the api wants. */
...
/* Which country the number is read as, which is what the form's own rule checks against. */
...
/** The national part alone: the dial code belongs to the picker. */
const typed = ref("")

/* A number arriving from the form is split back into the two parts it is typed as. */
watch(number, (value) => {
...
/* Only the countries libphonenumber knows a dial code for. */
...
<style scoped>
/* Two controls but one answer, so one box: two boxes read as two questions. */
.island-phone {
  display: flex;
  align-items: stretch;
  width: 100%;
