...
/**
 * What the panel found, read before it is opened. Drift is usually nothing, and a reader who
 * has to open every target to learn that stops opening any of them.
 */
const summary = computed<string>(() => {
  if (loading.value) return "checking…"
  if (error.value) return "could not be checked"
  const found = report.value
  if (found == null) return ""
  if (!found.externalCohortId) return "no external cohort yet"
  const parts: string[] = []
  if (found.extras.length > 0) parts.push(`${found.extras.length} extra`)
  if (found.missing.length > 0) parts.push(`${found.missing.length} missing`)
...
<template>
  <info-box
    ...
    label="Drift"
    :summary="summary"
    :testid="`cohort-drift-panel-${system.toLowerCase()}`"
  >
    <div>
      <v-alert
        v-if="error"
