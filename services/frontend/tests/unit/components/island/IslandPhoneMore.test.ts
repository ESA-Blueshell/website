...
const phone = (props: Record<string, unknown> = {}) =>
  mount(IslandPhone, {props: {modelValue: "", testidPrefix: "phone", ...props}, attachTo: document.body})
...
describe("a phone field", () => {
  ...
  it("rebuilds the number when another country is chosen", async () => {
    ...
    wrapper.findComponent({name: "IslandPicker"}).vm.$emit("pick", "DE")
  ...
  it("offers every country the api can dial, each with its code", () => {
    const wrapper = phone()
    const rows = wrapper.findComponent({name: "IslandPicker"}).props("options") as
