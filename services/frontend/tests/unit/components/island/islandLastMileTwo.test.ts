...
describe("a control with no name of its own", () => {
  it("names the parts it carries after the kind instead", () => {
    const phone = mount(IslandControl, {props: {kind: "phone", modelValue: ""}})
    expect(phone.findComponent({name: "IslandPhone"}).props("testidPrefix")).toBe("phone")

    const country = mount(IslandControl, {props: {kind: "country", modelValue: ""}})
    expect(country.findComponent({name: "IslandCountry"}).props("testidPrefix")).toBe("pick")

    const written = mount(IslandControl, {props: {kind: "markdown", modelValue: ""}})
    expect(written.findComponent({name: "IslandMarkdown"}).props("testid")).toBeUndefined()
  })
})
...
describe("a control passing what its part says back to the form", () => {
  it("keeps what the phone field and the editor write", async () => {
    const phone = mount(IslandControl, {props: {kind: "phone", modelValue: ""}})
    phone.findComponent({name: "IslandPhone"}).vm.$emit("update:modelValue", "+31612345678")
    await flushPromises()
    expect(phone.emitted("update:modelValue")?.at(0)).toEqual(["+31612345678"])

    const written = mount(IslandControl, {props: {kind: "markdown", modelValue: ""}})
    written.findComponent({name: "IslandMarkdown"}).vm.$emit("update:modelValue", "## Said")
    await flushPromises()
...
describe("the editor's own keys", () => {
  it("wraps what is selected in stars", async () => {
    const wrapper = mount(IslandMarkdown, {
  ...
  it("wraps what is selected in one star", async () => {
    const wrapper = mount(IslandMarkdown, {attachTo: document.body, props: {modelValue: "word"}})
    await flushPromises()
  ...
  it("says command where the machine is a Mac", async () => {
    platform("MacIntel")
    const wrapper = mount(IslandMarkdown, {attachTo: document.body, props: {modelValue: ""}})
  ...
  it("leaves the document alone where the value it is handed is the one it holds", async () => {
    const wrapper = mount(IslandMarkdown, {attachTo: document.body, props: {modelValue: "one"}})
    await flushPromises()
...
describe("a country field", () => {
  it("falls back to the country's name where nobody is called anything", async () => {
    ...
    expect(wrapper.findComponent({name: "IslandPicker"}).props("selectedKey")).toBe("BV")
  })
})

describe("a phone field", () => {
  it("keeps a number it was handed rather than writing it back", async () => {
    const wrapper = mount(IslandPhone, {
      attachTo: document.body,
...
describe("what a phone field is handed", () => {
  ...
  it("keeps the country it was on where the number names none", async () => {
    ...
    expect(wrapper.findComponent({name: "IslandPicker"}).props("selectedKey")).toBe("NL")
  })
  ...
  it("falls back to the country it holds where no row is under the pointer", () => {
    ...
    const slot = wrapper.findComponent({name: "IslandPicker"}).vm.$slots.chosen
