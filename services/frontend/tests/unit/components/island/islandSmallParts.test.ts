...
describe("IslandInput", () => {
  const input = (props: Record<string, unknown> = {}) =>
    mount(IslandInput, {props: {modelValue: "", ...props}})
...
describe("IslandField", () => {
  const field = (props: Record<string, unknown> = {}) =>
    mount(IslandField, {props: {label: "Email", ...props}, slots: {default: "<input>"}})
  ...
  it("points the control at what says it, only when there is something to say", () => {
    const quiet = mount(IslandField, {
      props: {label: "Email"},
      slots: {default: '<span :data-said="params.describedBy ?? \'none\'" />'},
    })
    const talking = mount(IslandField, {
...
describe("IslandCut", () => {
  const cut = (props: Record<string, unknown> = {}) =>
    mount(IslandCut, {props, slots: {default: "Go"}, global: {stubs: {RouterLink: RouterLinkStub}}})
...
describe("IslandCheck", () => {
  it("says its small print where it has any", () => {
    const plain = mount(IslandCheck, {props: {modelValue: false, label: "Newsletter"}})
    const noted = mount(IslandCheck, {
  ...
  it("is off when the form is, and carries a name when it is given one", () => {
    const wrapper = mount(IslandCheck, {
...
describe("IslandRadio", () => {
  ...
  it("says a hint where a choice has one, and none where it has not", () => {
    const wrapper = mount(IslandRadio, {props: {modelValue: "yes", options}})
  ...
  it("makes its own group name, and carries no testid where none was given", () => {
    const wrapper = mount(IslandRadio, {props: {modelValue: null, options}})
  ...
  it("is off when the form is", () => {
    const wrapper = mount(IslandRadio, {props: {modelValue: null, options, disabled: true}})
...
describe("IslandFile", () => {
  ...
  it("tells a picture, a pdf, a sheet, a page of text and anything else apart", () => {
    ...
    for (const [file, isImage] of shapes) {
      const wrapper = mount(IslandFile, {props: {modelValue: file}})
  ...
  it("reads a kind off the name where the browser gave none", () => {
    for (const name of ["rules.pdf", "members.csv", "notes.txt"]) {
      const wrapper = mount(IslandFile, {props: {modelValue: paper(name, "")}})
      expect(wrapper.find(".island-file__mark").exists()).toBe(true)
    }
  })

  it("says how heavy it is, in the unit that suits it", () => {
    const small = mount(IslandFile, {props: {modelValue: paper("a.pdf", "application/pdf", 500)}})
    const large = mount(IslandFile, {
