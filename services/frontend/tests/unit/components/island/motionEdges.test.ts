...
describe("what the parts time their movement by", () => {
  it("gives the cut button's sweep a length", () => {
    expect(cssVars(mount(CutButton, {props: {}, slots: {default: "Go"}}))).toContain("sweep")
  })
...
describe("what a control is named", () => {
  const control = (props: Record<string, unknown>) =>
    mount(FormControl, {props: {modelValue: "", ...props}})

  it("names the parts under it after the field, where the field has a name", () => {
    expect(control({kind: "phone", testid: "user-form-phone"})
      .findComponent({name: "PhoneInput"}).props("testidPrefix")).toBe("user-form-phone-phone")
    expect(control({kind: "country", testid: "user-form-country"})
      .findComponent({name: "CountryPicker"}).props("testidPrefix")).toBe("user-form-country-pick")
    expect(control({kind: "markdown", testid: "user-form-about"})
      .findComponent({name: "MarkdownEditor"}).props("testid")).toBe("user-form-about-editor")
  })

  it("falls back to a name of its own where the field has none", () => {
    expect(control({kind: "phone"}).findComponent({name: "PhoneInput"}).props("testidPrefix"))
      .toBe("phone")
    expect(control({kind: "country"}).findComponent({name: "CountryPicker"}).props("testidPrefix"))
      .toBe("pick")
    expect(control({kind: "markdown"}).findComponent({name: "MarkdownEditor"}).props("testid"))
      .toBeUndefined()
  })
  ...
  it("reports the code a picker chose", async () => {
    ...
    wrapper.findComponent({name: "CountryPicker"}).vm.$emit("update:modelValue", "FR")
    await wrapper.vm.$nextTick()
