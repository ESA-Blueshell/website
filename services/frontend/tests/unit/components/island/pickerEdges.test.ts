...
describe("the list a picker drops", () => {
  it("keeps the press that lands on itself", async () => {
    ...
    const list = document.querySelector("[data-picker-list]") as HTMLElement
    ...
    expect(document.querySelector("[data-picker-list]")).not.toBeNull()
  })
  ...
  it("stays down when a press lands on the panel drawn outside it", async () => {
    ...
    expect(document.querySelector("[data-picker-list]")).toBeNull()
  })
