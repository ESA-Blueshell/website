...
describe("the committees that organise events for a game", () => {
  ...
  it("offers the committees that run, and adds the ones picked", async () => {
    ...
    expect(picker(wrapper).props("options")).toEqual([{key: "1", label: "LanCie"}, {key: "2", label: "YapCie"}])
    picker(wrapper).vm.$emit("add", ["1"])
  ...
  it("keeps an archived committee already picked, drops one that is gone, and takes one away", async () => {
    ...
    expect(picker(wrapper).props("chosen")).toEqual([{key: "3", label: "OldCie"}, {key: "1", label: "LanCie"}])
    expect(picker(wrapper).props("chipTestid")("3")).toBe("game-organisers-3")
    picker(wrapper).vm.$emit("remove", "3")
