import logging
import os
import secrets
from typing import Optional

import httpx

from rotator import docker_ops
from rotator.config import Config
from rotator.env_files import update_env_file
from rotator.infisical import InfisicalClient, try_infisical_set
from rotator.rotators.base import Rotator

log = logging.getLogger(__name__)


class ListmonkRotator(Rotator):
    def rotate(self, env: str, stack: str, infisical: Optional[InfisicalClient], config: Config) -> None:
        log.info("[listmonk] rotating credentials for stack=%s", stack)

        listmonk_env = os.path.join(config.repo_root, "services/listmonk/.listmonk.env")
        old_user, old_pw = self._get_current_credentials(env, infisical, listmonk_env)
        new_pw = secrets.token_urlsafe(32)

        self._update_admin_password(config.listmonk_url, old_user, old_pw, new_pw)

        update_env_file(listmonk_env, "LISTMONK_ADMIN_PASSWORD", new_pw)
        try_infisical_set(infisical, "LISTMONK_ADMIN_PASSWORD", new_pw, env)

        # Delete the API token user so listmonk-setup regenerates it on next deploy
        docker_ops.exec_in_service(f"{stack}_listmonk-db", [
            "psql", "-U", "listmonk", "-c",
            "DELETE FROM users WHERE username='api' AND type='api';",
        ])
        docker_ops.remove_volume(f"{stack}_listmonk-secrets")

        docker_ops.stack_deploy(config.repo_root, stack)
        log.info("[listmonk] credentials rotated")

    @staticmethod
    def _get_current_credentials(
        env: str, infisical: Optional[InfisicalClient], listmonk_env: str
    ) -> tuple[str, str]:
        """Get current listmonk credentials from Infisical or local env file."""
        if infisical is not None:
            try:
                user = infisical.get("LISTMONK_ADMIN_USERNAME", env)
                pw = infisical.get("LISTMONK_ADMIN_PASSWORD", env)
                return user, pw
            except Exception:
                log.warning("Could not fetch Listmonk credentials from Infisical, trying local env")
        # Fall back to local env file
        user = "admin"
        pw = ""
        if os.path.isfile(listmonk_env):
            with open(listmonk_env) as f:
                for line in f:
                    if line.startswith("LISTMONK_ADMIN_USERNAME="):
                        user = line.split("=", 1)[1].strip()
                    elif line.startswith("LISTMONK_ADMIN_PASSWORD="):
                        pw = line.split("=", 1)[1].strip()
        if not pw:
            raise RuntimeError("LISTMONK_ADMIN_PASSWORD not found in Infisical or local env file")
        return user, pw

    def _update_admin_password(
        self, listmonk_url: str, username: str, old_pw: str, new_pw: str
    ) -> None:
        with httpx.Client() as client:
            client.post(
                f"{listmonk_url}/admin/login",
                data={"username": username, "password": old_pw},
                follow_redirects=True,
            ).raise_for_status()

            client.put(
                f"{listmonk_url}/api/profile",
                json={"password": new_pw, "password_confirm": new_pw},
            ).raise_for_status()
